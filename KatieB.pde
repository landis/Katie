...
const char PROGRAM_VERSION[16] = "KatieB V 1.0";
...
// speed steps and their delays [탎]:
const int NUM_SPEED_STEPS=6;
const unsigned long DELAYS[NUM_SPEED_STEPS] = {50000,20000,10000,5000,2000,1000};  // in 탎
const unsigned long HOMING_MOTOR_DELAY = 1000;  // defines the speed for the "return" = "go home" = "go to zero" command [탎]
...
/// So when e.g. you want a delay of 1000탎 between two actions, but some other code has consumed 200탎 already, call this function with delay_us=1000 and it will delay only 800탎.
...
/// @param delay_us How many 탎 to delay (since the last call).
...
void delaySinceLastCall(unsigned long delay_us, unsigned long &lastTime)
{
  ...
  do
  {
    ...
    if (m<lastTime) lastTime=m;  // reset when micros() has flown over (happens every 2^32 탎 = 71.6 minutes)
  }
  while (m-lastTime <= delay_us);  // loop until delay_us 탎 have passed
...
/// @param motorDelay Delay between rising and falling edges [탎]. Default HOMING_MOTOR_DELAY.
